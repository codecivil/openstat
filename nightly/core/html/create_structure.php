...
function _adminActionBefore(array $PARAMETER, mysqli $conn) {
	global $_TABLES, $_TABLES_ARRAY, $_TABLES_ALLOW_ARRAY, $_ROLES, $_ROLES_ARRAY, $_PARENTS, $_PARENTS_ARRAY, $ENCRYPTED;
	$_stmt_array = array();
	$_return = array();
	switch($PARAMETER['table']) {
		case 'os_roles':
...
<div id="content">
	...
	<form id="db_options" method="POST" action="">
		<fieldset>
			<legend></legend>
			<legend><a href="https://<?php echo($_SERVER['HTTP_HOST'].$_SERVER['SCRIPT_NAME'].'?table='.$PARAMETER['table']); ?>" title="Alle Parameter zurücksetzen">Alle Filter zurücksetzen</a></legend>
